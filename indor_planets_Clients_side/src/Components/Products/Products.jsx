  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <form onSubmit={handleSearch} className="flex justify-center mb-6 gap-2">
        ...
        />
        <button type="submit" className="btn cursor-target bg-gradient-to-r from-green-400 to-green-500 text-white">Search</button>
        {searchTerm && (
          <button type="button" onClick={handleClearSearch} className="btn btn-outline cursor-pointer cursor-target">Clear</button>
        )}
      </form>
      ...
      {products.length > 0 ? (
        ...
        <p className="text-center font-bold text-red-500  mt-6">No products found.</p>
      )}
        ...
        <div className="join flex cursor-target justify-center mt-10">
          ...
          {[...Array(totalPages).keys()].map((num) => (
            <button
              ...
              className={`join-item btn ${page === num + 1 ? 'btn-active bg-green-500 text-white' : 'border-2'}`}
            >
              {num + 1}
            </button>
          ))}
